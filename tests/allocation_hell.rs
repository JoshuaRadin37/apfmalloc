...
fn allocation_hell() {
    ...
    print!("Total Allocations to perform = {} [{} bytes -> {} bytes]", total_allocations, 1 << range.start, 1 << range.end);
    stdout().flush();
    for size in range.map(|shift| 1 << shift) {
        ...
        for i in 0..(ALLOCATIONS / 100) {
            ...
            do_free(ptr);
            print!("\u{8}\u{8}\u{8}\u{8}\u{8}\u{8}[{:3?}%]", (i as f64 / ALLOCATIONS as f64 * 100.0) as usize);
            stdout().flush();
        }
        for i in 0..(ALLOCATIONS - ALLOCATIONS / 100) {
            let mut ptr = do_malloc(size);
            do_free(ptr);
            print!("\u{8}\u{8}\u{8}\u{8}\u{8}\u{8}[{:3?}%]", ((i + ALLOCATIONS / 100) as f64 / ALLOCATIONS as f64 * 100.0) as usize);
            stdout().flush();
        }
